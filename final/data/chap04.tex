...
\begin{tikzpicture}[node distance=0.7cm]
    ...
    \node (sapi) [wmod, below=0.5em of mm]  {Hardware Abstract Layer};
    \node (hw) [wmod, below=0.5em of sapi, fill=red!20]  {Hardware};
    \node [below=0.8em of hw]  (SYS) {uCore kenrel space};
...
为了使uCore能适应多核NUMA硬件架构，对原有系统做多核扩展是，内存管理子系统（Memory
Management）和进程/线程管理子系统是最需要重大修改的地方。例如，对于NUMA系统，物理页分配器需要系统的内存/CPU拓扑结构制定分配策略；对于进程/线程调度系统，需要管理多核上的负载平衡和进程迁移。
